...
const sections = computed(() => tm('pages.privacy.sections') as LegalSection[])
  ...
  <div class="relative z-10">
    <section class="w-full flex flex-col items-center justify-center px-4 pt-24 pb-12 text-center">
      <div class="animate-fade-in-up">
        <h1 class="text-4xl sm:text-5xl font-bold font-serif text-white mb-4">{{ t('pages.privacy.heroTitle') }}</h1>
        <p class="text-gray-400 text-lg">{{ t('pages.privacy.heroSub') }}</p>
        <p class="text-gray-500 text-sm mt-2">{{ t('pages.privacy.updated') }}</p>
      </div>
    </section>
    ...
    <section class="w-full max-w-4xl mx-auto px-4 py-16 text-center reveal-on-scroll">
      <h3 class="text-2xl font-bold font-serif text-white mb-3">{{ t('pages.privacy.ctaTitle') }}</h3>
      <p class="text-gray-400 mb-8">{{ t('pages.privacy.ctaSub') }}</p>
      <RouterLink
        to="/yes-no-tarot"
      ...
      >{{ t('pages.privacy.ctaButton') }}</RouterLink>
    </section>
  </div>
</template>
